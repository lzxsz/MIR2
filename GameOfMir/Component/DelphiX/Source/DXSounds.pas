...
begin
  ...
  if FDirectMusicLoader.GetObject(FMusicObjDesc, IID_IDirectMusicSegment, FDirectMusicSegment) <> dd_ok then
    raise EDXMusicError.Create('Failed to Get object for Direct music'); ;
  if FDirectMusicSegment.setParam(GUID_StandardMidiFile, $FFFFFFFF, 0, 0, Pointer(FDirectMusicPerformance)) <> dd_ok then
    raise EDXMusicError.Create('Failed to Set param for Direct music'); ;
  if FDirectMusicSegment.setParam(GUID_Download, $FFFFFFFF, 0, 0, Pointer(FDirectMusicPerformance)) <> dd_ok then
    raise EDXMusicError.Create('Failed to Set param for Direct music'); ;
...
begin
  Result := False;
  if FDirectMusicPerformance <> nil then
    Result := FDirectMusicPerformance.IsPlaying (FDirectMusicSegment, FDirectMusicSegmentState) = DD_OK;
end;
...
begin
  ...
  if FDirectMusicPerformance = nil then
    OK := CoCreateInstance(CLSID_DirectMusicPerformance, nil, CLSCTX_INPROC,
      IID_IDirectMusicperformance, FDirectMusicPerformance) = DD_OK;
  if not OK then Exit;
  if FDirectSound <> nil then
    OK := FDirectMusicPerformance.Init(@FDirectMusic, FDirectSound, 0) = DD_OK
  else
    OK := FDirectMusicPerformance.Init(@FDirectMusic, nil, 0) = DD_OK;
  if not OK then Exit;
  // vychozi midi port
  // pridej pokud neni nastaven
  if FDirectMusicPerformance.Addport(nil) <> DD_OK then Exit;
  // zkus vytvorit loader
  OK := CoCreateInstance(CLSID_DirectMusicLoader, nil, CLSCTX_Inproc,
    IID_IDirectMusicLoader, FDirectMusicLoader) = DD_OK;
  FIsInitialized := OK;
end;
